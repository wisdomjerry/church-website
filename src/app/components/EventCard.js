...
export default function EventCard({ title, date, time, image, slug }) {
  // Fallback for dates that aren't "Month Day" format
      ...
      <div className="bg-white rounded-xl overflow-hidden shadow-sm group-hover:shadow-2xl transition-all duration-300 relative flex flex-col h-[55vh] border border-gray-100">
        ...
        <div className="p-8 pt-10 flex flex-col flex-1 bg-white">
          ...
          <div className="mt-auto pb-2">
            {/* We use a span here because the whole card is already a Link */}
            <span className="text-red-700 text-[10px] font-black uppercase tracking-widest border-b-2 border-red-700 pb-0.5 inline-block group-hover:text-gray-900 group-hover:border-gray-900 transition-all">
              Read More
            </span>
          </div>
        </div>
