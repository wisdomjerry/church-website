...
export default function Navbar() {
  ...
  const menuItems = [
    ...
    { name: "Ministries", href: "/ministries", hasDropdown: true },
  ...
  const ministries = [
    ...
    { name: "Married's Ministry", href: "/ministries/married" },
    ...
    <header className="fixed top-0 left-0 w-full z-[100]">
        ...
        <div className="hidden lg:flex bg-black/20 backdrop-blur-md text-white py-2 px-6 justify-between items-center border-b border-white/10 transition-all">
          <div className="flex gap-4 text-[10px] font-bold tracking-widest uppercase">
            <span className="flex items-center gap-1">
              <Phone size={14} className="text-red-700" /> +1 234 567 890
            </span>
            <span className="flex items-center gap-1">
              <Mail size={14} className="text-red-700" /> christainfaithharvestchurch@gmail.com
            </span>
          </div>
          <div className="flex gap-4">
            <Facebook size={16} className="hover:text-red-700 cursor-pointer transition" />
            <Youtube size={16} className="hover:text-red-700 cursor-pointer transition" />
          </div>
        </div>
      )}
      ...
      <nav className={`transition-all duration-500 border-b ${
        ...
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center h-16 relative">
          ...
          {/* Logo Section */}
          <Link href="/" className="z-[120] relative flex items-center gap-3 group">
            <div className={`p-1.5 rounded-full transition-all duration-500 ${isSticky || isOpen ? "bg-black/5" : "bg-white/10"}`}>
              <Image src="https://res.cloudinary.com/dnxnr4ocz/image/upload/v1769274333/favicon.ico_lwmtd1.png" width={35} height={35} alt="Logo" />
            </div>
            <div className={`text-[13px] md:text-[15px] font-black tracking-tighter leading-tight transition-colors duration-300 ${isSticky || isOpen ? "text-gray-900" : "text-white"}`}>
              <span className="text-red-700">CHRISTIAN</span> FAITH HARVEST <br className="md:hidden" /> CHURCH
            </div>
          </Link>
          ...
          <div className="hidden lg:flex space-x-8 items-center text-[11px] font-black uppercase tracking-widest">
            {menuItems.map((item) => (
              <div key={item.name} className="relative group/item">
                <Link
                  href={item.href}
                  className={`flex items-center gap-1 transition-colors duration-300 ${isSticky ? "text-gray-700" : "text-white"} hover:text-red-700 ${pathname.startsWith(item.href) && item.href !== '/' ? "text-red-700" : ""}`}
                >
                  {item.name}
                    ...
                    <div className="bg-white shadow-2xl border border-gray-100 min-w-[220px] py-4 rounded-sm">
                      {ministries.map((sub) => (
                        <Link
                          key={sub.name}
                          href={sub.href}
                          className="block px-6 py-3 text-gray-600 hover:text-red-700 hover:bg-gray-50 text-[10px] font-bold transition-colors border-l-2 border-transparent hover:border-red-700"
                        >
                          {sub.name}
                        </Link>
                      ))}
          ...
          {/* Hamburger Button */}
          <button onClick={() => setIsOpen(!isOpen)} className="lg:hidden flex flex-col justify-center items-end w-10 h-10 z-[120] outline-none group ml-auto">
